...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    function sortColumn(columnIndex) {
        ...
        if (lastColumn === columnIndex) direction = direction === "asc" ? "desc" : "asc";
        else direction = "asc";
        ...
        rows.sort((tr1, tr2)=>{
            ...
            return direction === "asc" ? clmn1.localeCompare(clmn2) : clmn2.localeCompare(clmn1);
        });
  ...
  `;
    ...
    form.addEventListener("submit", (e)=>{
        ...
        data.forEach((value, index)=>{
            ...
            if (index === 2) clmn.textContent = capitalizeWords(value);
            if (index === 4) clmn.textContent = toUsd(value);
            tr.append(clmn);
        });
        if (data[0].length < 4 || data[3] < 18 || data[3] > 90) return notification("error");
        if (columnName.includes(data[0])) return notification("warning");
        else {
            tbody.append(tr);
